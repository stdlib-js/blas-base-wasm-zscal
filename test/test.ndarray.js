...
tape( 'the `ndarray` method has an arity of 5', function test( t ) {
	t.strictEqual( zscal.ndarray.length, 5, 'arity of 5' );
	t.end();
});

tape( 'the `ndarray` method scales elements from `x` by `alpha`', function test( t ) {
	var expected;
	var viewX;
	var alpha;
	var x;
...
tape( 'the `ndarray` method supports specifying a `x` stride', function test( t ) {
	var expected;
	var viewX;
	var alpha;
	var x;
...
tape( 'the `ndarray` method supports specifying a negative `x` stride', function test( t ) {
	var expected;
	var viewX;
	var alpha;
	var x;
...
tape( 'the `ndarray` method supports a `x` offset', function test( t ) {
	var expected;
	var viewX;
	var alpha;
	var x;
...
tape( 'the `ndarray` method returns a reference to the input array', function test( t ) {
	...
	out = zscal.ndarray( 4, alpha, x, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the `ndarray` method returns the input array unchanged', function test( t ) {
	...
	zscal.ndarray( -1, alpha, x, 1, 0 );
	t.deepEqual( viewX, expected, 'returns expected value' );

	zscal.ndarray( 0, alpha, x, 1, 0 );
	t.deepEqual( viewX, expected, 'returns expected value' );
